...
use self::fonts::{FontRenderer, Fonts};
use math::*;
use Mat;
...
impl<'a> RenderContext<'a> {
    pub fn new(display: &'a Display) -> RenderContext<'a> {
        let font_renderer = FontRenderer::new(display);
...
fn rounded_rectangle(
    ...
    (tlr, trr, blr, brr): (f32, f32, f32, f32),
) -> (Vec<Vertex>, Vec<u32>) {
    fn create_corner(
    ...
    indices.extend(
        &[
            0, 3, 1, 1, 3, 4, 2, 1, 8, 8, 1, 7, 7, 1, 4, 7, 4, 10, 10, 4, 5, 10, 5, 11, 6, 7, 10,
            6, 10, 9,
        ][..],
    );
